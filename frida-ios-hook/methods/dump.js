...
function dumpModule(name) {
    ...
    if (targetmod == null) {
        console.log(colors.red,"Cannot find module",colors.resetColor);
        return;
    }
    ...
    if (fmodule == -1 || foldmodule == -1) {
        console.log(colors.red,"Cannot open file",colors.resetColor + newmodpath);
        return;
    }
...
function loadAllDynamicLibrary(app_path) {
    ...
    for (var i = 0, l = filenames.count(); i < l; i++) {
        ...
        if (file_name.hasSuffix_(".framework")) {
            ...
            if (bundle.isLoaded()) {
                console.log(colors.green,"[frida-ios-dump]: ",colors.resetColor + file_name + " has been loaded. ");
            } else {
                if (bundle.load()) {
                    console.log(colors.green,"[frida-ios-dump]: Load ",colors.resetColor + file_name + " success. ");
                } else {
                ...
                if (file_name.hasSuffix_(".dylib")) {
                    ...
                    if (!is_loaded) {
                        if (dlopen(allocStr(file_path.UTF8String()), 9)) {
                            console.log(colors.green,"[frida-ios-dump]: dlopen ",colors.resetColor + file_name + " success. ");
                        } else {
                            console.log(colors.red,"[frida-ios-dump]: dlopen ",colors.resetColor + file_name + " failed. ");
                        }
                    }
                }
            }
...
function handleMessage(message) {
    ...
    modules = getAllAppModules();
    for (var i = 0; i  < modules.length; i++) {
        console.log(colors.green,"[Start Dump]: ",colors.resetColor + modules[i].path);
